...
function add() {//添加
	window.location.href = web_domain+'/system/param_add.jsp';
}
...
function edit(){
	...
	window.location.href = web_domain+"/system/param_add.jsp?id="+$(ids[0]).val();
}
...
function query(page){
	$.ajax({
		  ...
		  data:{"page":page,"pageSize":pageSize,"type":"index"},
		  dataType: "jsonp",
		        	...
		        	$.each(json.result.list,function(n,obj){
		        		$("#listTable").append("<tr><td  height=\"30\" bgcolor=\"#FFFFFF\">" +
		        				"<input type=\"checkbox\" name=\"paramId\" id=\"checkbox2\" value=\""+obj.id+"\"/></td>" +
		        						...
		        						"<td height=\"30\" bgcolor=\"#FFFFFF\" class=\"STYLE19\">"+obj.order+"</td>" +
		        								"</tr>"
		        						);
		        	});
...
function del(){
    ...
    $.getJSON(server_param_domain+"m=del&ids="+paramId, function(json){ 
        ...
        if(code!=200){
        	...
        	window.location.href = web_domain+'/system/param_list.jsp?token='+token; 
        }
      });
}